...
#include "parsers/rscript_parser.h"
#include "parsers/json_parser.h"
#include "parsers/ptr_parser.h"
#include "parsers/error_parser.h"
#include "parsers/array_parser.h"
#include "parsers/lambda_parser.h"
#include "parsers/iter_parser.h"
#include "parsers/obj_parser.h"
#include "parsers/func_parser.h"
#include "parsers/func_parser.h"
#include <memory>
#include <stdexcept>
...
private:
    ...
    NodePtr createParser(const std::string& label,
        ...
        if (label == "rscript") return RScriptParser(block).parse();
        if (label == "json")    return JsonParser(block).parse();
        if (label == "ptr")     return PtrParser(block).parse();
        if (label == "error")   return ErrorParser(block).parse();
        if (label == "array")   return ArrayParser(block).parse();
        if (label == "lambda")  return LambdaParser(block).parse();
        if (label == "iter")    return IterParser(block).parse();
        if (label == "obj")     return ObjParser(block).parse();
        if (label == "func")    return FuncParser(block).parse();
        ...
        throw std::runtime_error(
            ...
            "@rscript, @json, @ptr, @error, @array, @lambda, @iter, @obj, @func");
    }
};
