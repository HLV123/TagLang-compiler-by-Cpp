...
struct DataEntryNode : ASTNode {
    ...
    std::string value;  // giá trị thô (string, number, bool)
    std::string dataType; // "string","int","float","bool","array"
    std::vector<std::string> arrayItems; // nếu là array
...
struct CalcExprNode : ASTNode {
    std::string lhs;   // biến lưu kết quả
    std::string expr;  // biểu thức thô (sẽ emit thẳng vào C++)
...
struct ExprNode : ASTNode {
    std::string raw; // biểu thức thô, emit thẳng
...
struct OutLineNode : ASTNode {
    std::string tmpl; // template string, {var} → biến
...
struct FlowStepNode : ASTNode {
    std::string from;
    std::string to;
    std::string condition; // optional: "if x > 0"
...
struct JsonFieldNode : ASTNode {
    std::string key;
    std::string value;
    std::string valueType; // string, int, float, bool, array
...
struct PtrPrintNode : ASTNode {
    std::string expr; // could be *p or p or var
